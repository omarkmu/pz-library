...
---@class omi.assert : luassert
---@field interpolate_match fun(patt: string, expected: string, tokens?: table, options?: table): boolean
local assert = assert
...
---@return boolean
local function interpolateMatch(state, arguments)
    local pattern = arguments[1]
    local expected = tostring(arguments[2])
    local tokens = arguments[3]
    local options = arguments[4]

    local result = interpolate(pattern, tokens, options)
    if result ~= expected then
        state.failure_message = format(
            'interpolation mismatch for %q (%q != %q)',
            tostring(pattern),
            expected,
            tostring(result)
        )
...
---Asserts that the given pattern and expected value match.
---@param pattern string
---@param expected string
local function runtimeInterpolateTest(pattern, expected)
    assert.interpolate_match(pattern, expected, mockTokens)
end

---Generates a test case for each provided pattern/result pair.
---@param cases unknown[][]
local function createInterpolateTests(cases)
    for _, case in pairs(cases) do
        local patt, expect = case[1], case[2]
        local name = #patt > 0
            and format('should yield the expected value for the pattern %q', patt)
            or 'should return the empty string for an empty pattern'

        it(name, utils.bind(runtimeInterpolateTest, patt, expect))
    end
end


say:set('assertion.interpolate_match.positive', 'Expected %s to match %s')
say:set('assertion.interpolate_match.negative', 'Expected %s not to match %s')
assert:register(
    'assertion',
    'interpolate_match',
    interpolateMatch,
    'assertion.interpolate.positive',
    'assertion.interpolate_match.negative'
)

describe('string interpolation', function()
    it('should respect the allowTokens option', function()
    ...
    it('should respect the allowFunctions option', function()
    ...
    it('should respect the allowCharacterEntities option', function()
